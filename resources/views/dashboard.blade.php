...
@section('content')
    <div class="max-w-7xl mx-auto">
        <!-- Greeting Section -->
        <div class="mb-6">
            <h2 class="text-2xl font-semibold text-gray-900 dark:text-white mb-1">{{ $greeting }},
                {{ $user->first_name ?? $user->username }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Here's what's happening in your literary world today.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                @if ($featuredBook)
                    <div
                        ...
                        <div class="flex flex-col sm:flex-row">
                            <div
                                class="sm:w-2/5 h-48 sm:h-auto relative overflow-hidden bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30">
                                ...
                                @else
                                    <div class="w-full h-full flex items-center justify-center">
                                        <i class="bx bx-book text-6xl text-blue-300 dark:text-blue-600"></i>
                                    </div>
                                @endif
                            </div>
                            <div class="sm:w-3/5 p-5">
                                <span
                                    class="inline-block px-2.5 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-[11px] font-semibold rounded-md mb-3 uppercase tracking-wide">Book
                                    of the Day</span>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                                    {{ $featuredBook->title }}</h2>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">by {{ $featuredBook->author }}</p>
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <a href="{{ route('books.show', $featuredBook->id) }}"
                                        class="px-4 py-2 bg-blue-50 dark:bg-gray-800 text-blue-600 dark:text-blue-400 text-sm font-medium rounded-lg hover:bg-blue-100 dark:hover:bg-gray-700 transition-colors flex items-center gap-1.5">
                ...
                <!-- Recommended for You -->
                <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-5 border border-gray-100 dark:border-gray-800">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            @foreach ($recommendedBooks as $book)
                                <a href="{{ route('books.show', $book->id) }}" class="flex-shrink-0 group">
                                    <div
                                        class="w-full aspect-[2/3] bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg mb-2 overflow-hidden shadow-sm group-hover:shadow-md transition-shadow">
                                        @if ($book->coverImage)
                                    ...
                                    <h3
                                        class="text-xs font-medium text-gray-900 truncate group-hover:text-blue-600 transition-colors">
                                        {{ $book->title }}</h3>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
                        ...
                        @forelse($trendingPoems as $poem)
                            <a href="{{ route('poetry.show', $poem->id) }}"
                                class="flex items-start gap-3 {{ !$loop->last ? 'mb-4 pb-4 border-b border-gray-100' : '' }} hover:bg-gray-50 -mx-2 px-2 py-2 rounded-lg transition-colors">
                                <div
                                    class="w-10 h-10 bg-blue-50 rounded-full flex items-center justify-center flex-shrink-0">
                                    <i class="bx bx-pen text-blue-600"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <span
                                        class="text-sm font-medium text-gray-900 block truncate">{{ $poem->title }}</span>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <a href="{{ route('creators.index') }}"
                                class="text-xs text-blue-600 hover:text-blue-700 font-medium">Find More</a>
                        </div>
                        @forelse($followedCreators as $creator)
                            <a href="{{ route('profile.creator', $creator->id) }}"
                                class="flex items-start gap-3 {{ !$loop->last ? 'mb-4 pb-4 border-b border-gray-100' : '' }} hover:bg-gray-50 -mx-2 px-2 py-2 rounded-lg transition-colors">
                                <div
                                    class="w-10 h-10 bg-gradient-to-br from-pink-400 to-purple-500 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                                    <span
                                        class="text-white text-sm font-medium">{{ strtoupper(substr($creator->first_name ?? $creator->username, 0, 2)) }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <span
                                        class="text-sm font-medium text-gray-900 block truncate">{{ $creator->first_name ?? $creator->username }}
                        ...
                        @empty
                            ...
                            <a href="{{ route('creators.index') }}"
                                class="block text-center text-sm text-blue-600 hover:text-blue-700 font-medium mt-2">Discover
                                Creators</a>
                        @endforelse
                    </div>
                </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-xs text-blue-600 font-medium bg-blue-50 px-2 py-0.5 rounded-full">{{ $liveChatrooms->count() }}
                            Active</span>
                    </div>
                    <div class="space-y-4">
                        @forelse($liveChatrooms as $chatroom)
                            <a href="{{ route('chatroom.show', $chatroom->id) }}"
                                class="block {{ !$loop->last ? 'pb-4 border-b border-gray-100' : '' }}">
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="flex -space-x-1.5">
                                        @for ($i = 0; $i < min(3, $chatroom->members_count); $i++)
                                            <div
                                                class="w-5 h-5 bg-{{ ['blue', 'purple', 'pink'][$i] }}-500 rounded-full border-2 border-white">
                                            </div>
                                        @endfor
                                    </div>
                                    @if ($chatroom->members_count > 3)
                                        <span class="text-[11px] text-gray-500">+{{ $chatroom->members_count - 3 }}
                                            others</span>
                                    @endif
                                </div>
                            </a>
                    ...
                    <a href="{{ route('chatrooms.index') }}"
                        class="block w-full mt-4 px-4 py-2 bg-gray-50 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-100 transition-colors text-center">
                        View All Rooms
                    </a>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-5 border border-gray-100">
                    ...
                    <div class="space-y-4">
                        @forelse($upcomingEvents as $event)
                            <div class="flex gap-3">
                                <div
                                    class="w-12 h-14 bg-blue-50 rounded-lg flex flex-col items-center justify-center flex-shrink-0">
                                    <span class="text-lg font-bold text-blue-600">{{ $event->date->format('d') }}</span>
                                    <span
                                        class="text-[10px] text-blue-600 uppercase">{{ $event->date->format('M') }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                    ...
                    <a href="{{ route('events.index') }}"
                        class="block w-full mt-4 text-xs text-blue-600 hover:text-blue-700 font-medium text-center">
                        View All Events
                    </a>
                </div>
            </div>
