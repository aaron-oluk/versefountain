...
@section('content')
    <div class="min-h-screen bg-white dark:bg-gray-900 flex flex-col">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1">
            <!-- Search Bar -->
            <div class="mb-8">
                <div class="relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        @foreach ($featuredResources as $resource)
                            <a href="#" class="group">
                                ...
                                <h3 class="font-semibold text-gray-900 dark:text-white mb-2 line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $resource->title }}</h3>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ $resource->subject ?? 'Academic' }}</p>
                                <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-500">
                                    <span>{{ $resource->created_at->format('M d, Y') }}</span>
                                    <span class="px-2 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded font-medium">{{ strtoupper($resource->type ?? 'PDF') }}</span>
                                </div>
                            </a>
                        @endforeach
                    </div>
                        ...
                        @foreach ($subjects as $subject)
                            <a href="#" class="group p-4 rounded-lg border border-gray-200 dark:border-gray-800 hover:border-blue-500 dark:hover:border-blue-400 bg-white dark:bg-gray-800 transition-all">
                                <div class="text-center">
                                    <div class="w-12 h-12 rounded-lg bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center mx-auto mb-3 group-hover:bg-blue-100 dark:group-hover:bg-blue-800/40 transition-colors">
                                        <i class="bx bx-book-bookmark text-xl text-blue-600 dark:text-blue-400"></i>
                                    </div>
                                    <p class="text-sm font-medium text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $subject }}</p>
                                </div>
                            </a>
                        @endforeach
                    </div>
                ...
                @if ($recentPapers->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                        @foreach ($recentPapers as $paper)
                            <a href="#" class="group p-5 rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 hover:border-blue-500 dark:hover:border-blue-400 hover:shadow-md dark:hover:shadow-lg transition-all">
                                <div class="flex items-start gap-4 mb-4">
                                    <div class="w-12 h-12 rounded-lg bg-purple-50 dark:bg-purple-900/20 flex items-center justify-center flex-shrink-0 group-hover:bg-purple-100 dark:group-hover:bg-purple-800/40 transition-colors">
                                        <i class="bx bx-file-pdf text-xl text-purple-600 dark:text-purple-400"></i>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-semibold text-gray-900 dark:text-white line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $paper->title }}</h3>
                                        <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ $paper->subject ?? 'General' }}</p>
                                    </div>
                                </div>
                                <p class="text-sm text-gray-700 dark:text-gray-300 mb-4 line-clamp-2">{{ $paper->description ?? 'No description available' }}</p>
                                <div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                                    <span class="text-xs text-gray-500 dark:text-gray-500">{{ $paper->created_at->format('M d, Y') }}</span>
                                    <span class="text-sm font-semibold text-blue-600 dark:text-blue-400 group-hover:gap-2 flex items-center gap-1 transition-all">
                                        Download
